...
The approved Adsterra inventory contains one responsive display placement per eligible page plus a matching Social Bar. Discovery pages and game-detail pages use separate reporting batches. Every provider script is optional-consent gated. Active games, the Living Shelf, legal/support routes, offline/error pages, and not-found pages contain no ad mount. Popunder and clickunder formats are permanently excluded. See `ADSTERRA-STARTER-BATCH-2026-07-22.md` for the route map and replacement plan. Account credentials and provider secrets must never be committed.
